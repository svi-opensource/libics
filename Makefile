...
pkgdatadir = $(datadir)/libics
pkglibdir = $(libdir)/libics
pkgincludedir = $(includedir)/libics
top_builddir = .
...
am__aclocal_m4_deps = $(top_srcdir)/m4/etr_string_strcasecmp.m4 \
	$(top_srcdir)/m4/etr_strings_strcasecmp.m4 \
...
CONFIG_HEADER = config.h libics_conf.h
CONFIG_CLEAN_FILES =
...
SOURCES = $(libics_la_SOURCES)
DIST_SOURCES = $(libics_la_SOURCES)
...
libics_conf.h: stamp-h2
	@if test ! -f $@; then \
	  rm -f stamp-h2; \
	  $(MAKE) stamp-h2; \
	else :; fi

stamp-h2: $(srcdir)/libics_conf.h.in $(top_builddir)/config.status
	@rm -f stamp-h2
	cd $(top_builddir) && $(SHELL) ./config.status libics_conf.h

distclean-hdr:
	-rm -f config.h stamp-h1 libics_conf.h stamp-h2
...
libics.la: $(libics_la_OBJECTS) $(libics_la_DEPENDENCIES) 
	$(LINK) -rpath $(libdir) $(libics_la_LDFLAGS) $(libics_la_OBJECTS) $(libics_la_LIBADD) $(LIBS)
...
include ./$(DEPDIR)/libics_preview.Plo
include ./$(DEPDIR)/libics_read.Plo
include ./$(DEPDIR)/libics_sensor.Plo
include ./$(DEPDIR)/libics_test.Plo
include ./$(DEPDIR)/libics_top.Plo
include ./$(DEPDIR)/libics_util.Plo
include ./$(DEPDIR)/libics_write.Plo
...
TAGS:  $(HEADERS) $(SOURCES) config.h.in libics_conf.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	tags=; \
	here=`pwd`; \
	list='$(SOURCES) $(HEADERS) config.h.in libics_conf.h.in $(LISP) $(TAGS_FILES)'; \
	unique=`for i in $$list; do \
	    if test -f "$$i"; then echo $$i; else echo $(srcdir)/$$i; fi; \
	  done | \
	  $(AWK) '    { files[$$0] = 1; } \
...
ctags: CTAGS
CTAGS:  $(HEADERS) $(SOURCES) config.h.in libics_conf.h.in $(TAGS_DEPENDENCIES) \
		$(TAGS_FILES) $(LISP)
	tags=; \
	here=`pwd`; \
	list='$(SOURCES) $(HEADERS) config.h.in libics_conf.h.in $(LISP) $(TAGS_FILES)'; \
...
distdir: $(DISTFILES)
	$(am__remove_distdir)
	mkdir $(distdir)
	$(mkdir_p) $(distdir)/Documentation $(distdir)/m4 $(distdir)/support/icsviewer $(distdir)/support/matlab
...
check-am: all-am
check: check-am
all-am: Makefile $(LTLIBRARIES) $(HEADERS) config.h libics_conf.h
installdirs:
